async function loadConfig(){
  const r=await fetch('/api/clinical/config/'); const d=await r.json();
...
document.getElementById('ask').addEventListener('click', async ()=>{
 ...
 const resp=await fetch('/api/clinical/ask/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
